...
function ChatModal({ coordinator, student, open, onClose }) {
  ...
  let listChatMessages = chatMessages.map((chatMessage, index) => (
    <ListItem key={index}>
      <ListItemText primary={`${chatMessage.fromId === coordinator.id? coordinator.firstName + " " + coordinator.lastName : student.firstName + " " + student.lastName}: ${chatMessage.text}`} />
    </ListItem>
  ));
